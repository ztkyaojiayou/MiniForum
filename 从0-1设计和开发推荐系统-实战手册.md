...
### 3.6 三域 × 三层：谁在什么时候算（计算时机对照）

把"三域（算什么）" × "三层（何时算/在哪层算）"放一张表，就看清每个信号由谁在什么时机产出、滞后多少：

| 三域 \ 三层 | **在线**（请求路径） | **近线**（秒~分钟） | **离线**（定期批） |
|---|---|---|---|
| **画像** UserProfile | ✅ **读时懒构建**：`aggregator.build` 从行为日志全量现算 + TTL（demo 30s / prod Redis 1h）。**唯一算完整画像的地方** | ⚠️ 只给"近 N 分钟话题点击投影" `realtime:user` topicClicks（实时兴趣片段，非完整画像） | ❌ 不算画像（离线只算 ItemCF 模型 + 离线评估） |
| **特征** | ✅ **ItemFeature**：内容计数特征（hotScore 等）读时现算 + 5s TTL（ItemFeatureFormula，§3.2） | ✅ **RealtimeFeature**：近线窗口滚动（demo `RealtimeFeatureWindow` 每 5s flush / prod Flink 独立作业）→ 精排 `realtime` 特征读取 | ✅ **ItemCF 模型**：离线每 30min 重建（物品↔物品相似度，O(物品²) 重活） |
| **图谱** SocialGraph | ✅ 读时实时查（isFollowing / followedRepostedIds / authorFollowers，作者粉丝带 5s 短缓存） | ❌ | ❌（follow 关系是事实源，图查询在线现算即可） |

**关键认知**：
1. **三域"更新时机"天然不同**：画像/ItemFeature/图谱是**慢变量** → 在线读时算 + TTL 近实时；RealtimeFeature 是**真实时窗口** → 近线滚动；ItemCF 是**最重的批** → 离线定期；
2. **画像为什么不走近线/离线**：规模小 + TTL（30s/1h）够用。大厂画像常是"离线批量预算 + 近线增量"双轨，本项目刻意简化成**在线读时全量重算**（见 §4 首条）；
3. **demo vs prod 的近线差异**：demo 用 `RealtimeFeatureWindow` 内存模拟整条近线链（**零 Flink**）；prod 才由独立 Flink 作业写 Redis——精排的 `realtime` 特征两套都能读到，**接入是通的**（不是"没接、要 Flink 才接"）；
4. **近线 ≠ 画像**：`realtime:user` 的 topicClicks 只是"最近 5 分钟你在点啥"，与 `UserProfile.topicWeight`（长期全量话题权重）是**两套信号**，别混为一谈。
