...
`forum-core/recommend/stream/` 定义领域事件与总线接口（`PostCreatedEventBus`、`PostCreatedEvent`、`PostCreatedNotifier`、`OutboxStore`、`InMemoryOutboxStore`、`BehaviorEventQueue`）。订阅者位于 `forum-recommend-server/recommend/stream/`（`FanoutOnPostCreated` 分发到 `TrafficPoolOnPostCreated` 预热冷启动池、`SearchIndexUpdater` 建搜索索引，以及内容管道等下游消费者），对标生产环境 Kafka 将一份 `post-created` 事件广播给多个独立消费组。新增下游消费者应通过总线接线，而非直接调用 repository。
