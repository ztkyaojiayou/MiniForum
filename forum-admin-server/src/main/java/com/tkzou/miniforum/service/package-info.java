 ...
 * <b>关键数据流转</b>：
 * <pre>
 * 发帖：PostService.createPost → 规范化/话题提取/@提及 → save → postCreatedProducer.notify（事件，扇出/冷启）
 ...
 *   <li>发帖事件只依赖 {@code PostCreatedProducer} 接口（forum-core），实现（内存/Kafka）在 recommend 侧；</li>
