...
>
> **当前是"钩子就位、读侧缺失"的精确状态**：`shouldSkipFanout` 与阈值（默认 10 万）都实现了，但读侧 pull 合并没实现——若真把某作者粉丝推到阈值，他的新帖会直接消失在粉丝流里。要激活，缺的是混合模式骨架：
> - **写侧**：大 V 不发扇出，只写进自己的时间线（`postRepository.findByAuthorId`，即 `recentPostIdsOf`）；
> - **读侧**：粉丝刷流 = 自己的 inbox（推）+ 所有已关注大 V 的近期帖（拉），两部分按 postId 合并排序再分页；
> - **分页复杂度**：现在是"单个 inbox 一次游标"，合并后要"推流游标 + 每个大 V 流游标"交叉取——实现比现在复杂一个量级，这也是它被明确标"预留"、没进 P0/P1 的原因。
...
**大V（粉丝海量，百万级）**
- **写（发帖）**：扇出 = 百万次 inbox 写入 → **写爆炸**（就是 3.3 的 `shouldSkipFanout` 要解决的）；
- **读（刷流）**：自己的流仍是 O(1) 便宜；
