...
> 本章目标：理解微博业务的**数据底座**——10 个实体是什么、关系怎么建、**为什么这样建模**（尤其两个自关联 + 连接表）。
...
**建模原则**：实体承载"事实"，关系用"外键字段"或"连接表"表达，计数（likeCount）是**聚合快照**不是事实本身。

## 2. 核心类清单（10 个实体）

| 实体 | 一句话职责 | 关键字段 | 说明 |
|---|---|---|---|
| `User` | 账号主体 | id, username, password, nickname | 业务上 username 唯一 |
| `Post` | 内容单元 | id, content, authorId, tags[], topics[], category, status, likeCount, viewCount, **originalPostId** | 可被转发 |
| `Follow` | 关注关系（有向边） | followerId, followeeId | **User 自关联的连接表** |
| `Like` | 点赞 | postId, username | 一人一帖一赞（唯一约束） |
| `Favorite` | 收藏 | postId, username | 同 Like |
| `Comment` | 评论 | postId, author, content, **parentId** | 支持楼中楼 |
| `Conversation` | 私信会话 | userA, userB | **字典序去重**保证两人唯一 |
| `Message` | 私信消息 | conversationId, senderId, receiverId, read | 属于某会话 |
| `Notification` | 通知 | recipientId, actorId, type, postId, read | 事件驱动生成 |
| `SearchRecord` | 搜索词聚合 | keyword, count | 喂热搜榜 |

## 3. 关键逻辑：三个最值得讲的建模决策

### 3.1 Post 自关联 = 微博转发的本质
...
```
> 观察：一次互动牵动了 Post（计数）/Like（事实）/Notification（触达）三个实体——这就是"实体关系"在运行时的样子。
...
| `PostCreatedNotifier` | 发帖事件发布器接口 | `notify(PostCreatedEvent)` |
...
发帖事务：{ 插入 Post } + { 插入 Outbox }    ← 同一事务，原子（at-least-once）
  → Relayer 扫描 Outbox → 发布 PostCreatedEvent（演示=内存总线；生产=Kafka "post-created"）
    → 订阅者① FanoutOnPostCreated：扇出到关注流 inbox
    → 订阅者② TrafficPoolOnPostCreated：新帖入冷启流量池
...
```java
// FanoutOnPostCreated：发帖事件 → 扇出到粉丝关注流（推模式，第 5 章）
@Component
public class FanoutOnPostCreated {
    public FanoutOnPostCreated(PostCreatedEventBus bus, FollowFeedStore followFeedStore) {
        // 构造器即订阅：发帖事件一到 → 把 postId 写进每个粉丝的 inbox
        bus.subscribe(event -> followFeedStore.fanout(event.getAuthorId(), event.getPostId()));
    }
}
// 别的订阅者：TrafficPoolOnPostCreated（新帖入冷启池）/ SearchIndex（搜索索引增量）——
...
- 事件总线（PostCreatedEventBus）≈ Kafka 的单机版——一份事件多路消费，和微服务的发布-订阅一致。
...
```
本项目用**推模式**：读快（微博首页要秒开）、写慢一点可接受；代价是"写放大"（大 V 千万粉丝扇出巨大）——所以预留了大 V 分流开关（`shouldSkipFanout`，粉丝数超阈值跳扇出走拉模式，本项目只留开关未实现拉模式）。
...
| `FollowFeedStore` | 关注流 inbox（推模式，接口 + 内存/Redis 双实现） | fanout(authorId, postId), getInbox, onFollow(回填), shouldSkipFanout(大V) |
| `FanoutOnPostCreated` | 发帖事件订阅者：收到 PostCreatedEvent → fanout 到粉丝 | 构造器即订阅 |
...
② 扇出：FanoutOnPostCreated 订阅 → followFeedStore.fanout(authorId, postId)
...
// RuleRankService / FollowRecall 用：
...
**为什么"编排器"而非"大而全"**：每个环节（召回/排序/重排）是独立可替换的模块（接口 + 实现分离）——想换排序算法，只换 `RuleRankService`，不动编排器。这是推荐系统可演进的前提（第 16 章双实现、第 17 章微服务都依赖这个解耦）。
...
**③ 事件订阅者由 Spring 自动收集（`PostCreatedSubscriberRegistrar`）**
```java
// 任何实现 PostCreatedSubscriber 的 @Component，都会自动被注册进发帖事件总线
@Component class FanoutOnPostCreated       implements PostCreatedSubscriber {...} // 关注流扇出
@Component class SearchIndexUpdater        implements PostCreatedSubscriber {...} // 建搜索索引
@Component class TrafficPoolOnPostCreated implements PostCreatedSubscriber {...} // 冷启池预热
...
RecommendService
  ...
  ├─ RankService           （排序）   ─ RuleRankService
...
- 大厂召回 = 多个召回引擎（向量召回双塔 / 协同过滤 / 规则路）+ **粗排**（轻模型把千级缩到百级）。本项目 6 路规则召回直接融合，**省掉了粗排**（候选量小，排序直接上）——这是量级的简化，不是架构缺环；
...
**为什么规则加权而非模型**：弱训练侧（无训练数据管线）。规则加权的优点：可解释、零训练、改权重即调参；缺点：特征交叉靠人设计。**它的架构（特征→加权→排序）和深度模型完全一致，只是"权重从哪来"不同**——换模型时只换 `RuleRankService` 内部打分，接口不动。
...
| `RuleRankService` | 规则加权默认实现 | 7 特征 + rankScore + buildExplain |
...
**RuleRankService.rank 真实代码**（对每个候选算 7 特征 + 加权，数据流最核心的一段）：

```java
// RuleRankService.rank —— 排序：候选 → 特征分 → 加权 → rankScore
...
| `TrafficPoolOnPostCreated` | 发帖事件 → 新帖入流量池 | PostCreatedEventBus 订阅者 |
...
| **粗排层** | 召回→精排直接 | 召回后加轻量粗排（千→百）控延迟 | docs/微服务拆分方案 |
...
| 类 | 一句话职责 | 关键方法 |
|---|---|---|
| `FollowFeedStore` | 推模式 inbox 接口 | `fanout(authorId, postId)`, `getInbox(userId, maxId, maxCount)`, `getInboxAfter`, `onFollow`, `isBuilt`, `shouldSkipFanout` |
| `InMemoryFollowFeedStore` | 内存 inbox（每用户一个有序集合） | @Profile("!prod") |
| `RedisFollowFeedStore` | 生产 inbox（ZSET `feed:inbox:{uid}`） | @Profile("prod") |
| `FanoutOnPostCreated` | 发帖事件 → fanout 的订阅者 | 构造器即订阅（第 3 章） |
...
发帖（作者）
  ...
  → FanoutOnPostCreated 订阅 → followFeedStore.fanout(authorId, postId)
...
public interface FollowFeedStore {
    void fanout(Long authorId, Long postId);                      // 写侧：发帖喂粉丝
    List<Long> getInbox(Long userId, Long maxId, int maxCount);   // 读侧：向下游标翻页
    List<Long> getInboxAfter(Long userId, Long sinceId, int maxCount); // 增量刷新（新帖提示）
    void onFollow(Long followerId, List<Long> recentPostIds);     // 关注回填
    boolean isBuilt(Long userId);                                 // 建流标记（没建不扇出）
    boolean shouldSkipFanout(Long userId);                        // 大V分流（预留）
}
...
### 3.3 大 V 分流（写放大的解药，本项目预留开关）

```java
// shouldSkipFanout：粉丝数超阈值 → 跳过扇出（大 V 的 inbox 写不动了）
public boolean shouldSkipFanout(Long userId) {
    return followRepository.countByFolloweeId(userId) >= bigVThreshold;  // 默认 10 万
}
// 触发后：大 V 新帖不进粉丝 inbox → 读侧需要"拉模式合并"补回来
// ⚠️ 本项目只留了开关、未实现拉模式——真触发会丢大 V 新帖，属预留状态（docs/feed流架构调研与对比.md 有完整方案）
```
...
- **每用户时间线**：推模式 = 每个用户一个"时间线缓存"（inbox），Redis ZSET 是标配（score=时间戳）；本项目 `feed:inbox:{uid}` 就是它；
- **游标分页**：大厂 feed 都用 `max_id`/`since_id` 游标（而非页码），因为新帖插入会打乱页码——本项目 `getFollowFeed(maxId)` 对齐了这个；
- **写放大是永恒的课题**：本项目大 V 分流预留（`shouldSkipFanout`）是混合模式的骨架，完整实现（拉模式合并、粉丝数分档）见 docs/feed流架构调研与对比.md。
...
5. **大 V 开关触发但没拉模式**：`shouldSkipFanout` 触发会丢大 V 新帖——要么别触发（保持全推），要么实现拉模式合并。
...
**7 个特征 = 7 个"为什么给分"**（`RuleRankService`），P88 全真数据：
...
| **粗排 / 精排** | 大厂在召回与精排间再加一层轻量粗排控延迟；本项目省掉粗排直接精排 | 09/10 |
