 ...
 * 帖子创建事件订阅者装配器（统一注册入口）
 * <p>
 * 利用 Spring 的 {@code List<接口>} 自动收集机制，注入容器内全部 {@link PostCreatedConsumer}
 ...
 */
@Component
public class PostCreatedConsumerRegistrar {

    public PostCreatedConsumerRegistrar(PostCreatedEventBus eventBus,
                                          List<PostCreatedConsumer> subscribers) {
        for (PostCreatedConsumer subscriber : subscribers) {
            eventBus.subscribe(event -> subscriber.consume(event));
        }
    }
}