...
1. **主流系统的高并发 = 四板斧**：漏斗削减 + 预计算/缓存 + 异步削峰 + 熔断降级兜底。微博/抖音/头条/小红书无一例外，只是侧重不同（微博重缓存与热点、字节重"轻在线重离线"与模型服务化、头条重存储与大扇出、小红书重近线调度降本）。
...
3. **最大差距不是缺中间件，而是缺"在线层的算力治理"**：多级缓存、画像/特征预聚合、限流熔断降级、热点处理。中间件（Kafka/Redis/ClickHouse/MySQL）本项目已齐；把这些在线层机制补上，才谈得上"真高并发"。
4. **补全的优先级很清晰**：先"让单条请求算得更便宜 + 峰值有预案"（3.1~3.3），再"能水平扩展"（3.5），最后"可观测可压测"（3.6）。当前阶段（演示/教学）**不需要**做读写分离、分库分表、异地多活——量级未到，且会显著增加复杂度（业界共识：能不分尽量不分）。
