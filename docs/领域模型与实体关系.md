...
> 基于源码核验：`forum-core/entity` + `forum-core/repository` + `recommend/behavior` + demo-runner `src/prod`。
...
```
实体层（entity, 纯 POJO，逻辑外键）→ 仓储层（repository, 接口 + InMemory 默认 / MySql prod）→ 视图层（dto, PostAssembler 装配）
```
...
> 七个实体接口定义在 `forum-core/repository/`，内存实现 `@Profile("!prod")`、MySQL 实现 `@Profile("prod")`（demo-runner `src/prod`）。**P1.1 已含全部 10 实体**——Message/Conversation/Notification/SearchRecord 在 `7c65574` 收官补齐。
