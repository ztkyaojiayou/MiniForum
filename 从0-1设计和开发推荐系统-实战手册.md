...
> **内存版 MQ 概念映射（本项目的"零中间件 Kafka"）**——把散落组件按 producer / bus / consumer 三个角色理解就清楚了：
>
...
> | **Bus（内存版 MQ）** | `PostCreatedEventBus` | `BehaviorEventQueue` |
> | **Consumer（消费者）** | `PostCreatedConsumer`（FanoutOnPostCreated / SearchIndexUpdater / TrafficPoolOnPostCreated） | `Consumer<BehaviorLog>` 订阅（实时窗口 / 冷启动反馈 / 流量池 / 热搜） |
...
**③ 事件订阅者由 Spring 自动收集（`PostCreatedConsumerRegistrar`）**
```java
// 任何实现 PostCreatedConsumer 的 @Component，都会自动被注册进发帖事件总线
@Component class FanoutOnPostCreated       implements PostCreatedConsumer {...} // 关注流扇出
@Component class SearchIndexUpdater        implements PostCreatedConsumer {...} // 建搜索索引
@Component class TrafficPoolOnPostCreated implements PostCreatedConsumer {...} // 冷启池预热
