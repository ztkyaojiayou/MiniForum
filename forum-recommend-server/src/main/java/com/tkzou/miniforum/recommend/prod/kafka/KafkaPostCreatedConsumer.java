 ...
 * <p>
 * <b>两跳消费者说明</b>：这是<b>Kafka 一跳</b>的消费者（读外部 MQ → 灌进内存总线），
 * <b>不实现 {@code PostCreatedConsumer} 接口</b>——该接口是<b>总线一跳</b>的领域消费者契约
 * （Fanout/SearchIndex/TrafficPool 等处理已投递事件）。两条跳点都是"消费事件"，只是消费的队列不同。
 */
@Component
@Profile("prod")
